...
## 11. Revamp, 2026-08-22

Supersedes §3, §6, §7, §8 and §10 where they differ.

- Direction: the product is the hero. A dark band carries a real terminal recording of `tess` (vhs against a throwaway vault, `public/media/hero.{mp4,webm}`, 1000x560, poster = frame 0 = the `tess watch` view; tapes and the recording script in `scripts/`). The app section carries a live window capture of the real app (`src/assets/app-vault-{light,dark}.png`, 1440x1044 at 2x, from `docs/appstore-assets/captures/`). No CSS mocks: `TileMosaic.astro` and `Terminal.astro` are gone, and with them the tessellation load animation and scroll reveals.
- Layout: one page grid in `global.css` (`.page`, named lines full / wide / content): text gets a measure (60ch), media does not. Sections opt into `.u-wide`, `.u-full`, `.u-bleed-right`; `.band` re-exposes the grid with its own ground. No `max-w-[var(--maxw)]` wrappers.
- Rhythm on `/`: ink hero / sunken install / cream app (the window is cut by the viewport edge and overlaps the next band) / sunken wide one-vault / cream security rows / sunken compare table / cream close / ink footer. Heading sizes: h1 display, then section, section, section, sub, display on the close. Document pages (`/cli`, `/security`, `/vault-format`, `/privacy`, `/changelog`, `/learn`) keep one h2 size on purpose: their sections are peers.
- Type: 400 body (Hanken Grotesk) and 700 display (Schibsted Grotesk), wordmark included; IBM Plex Mono 400 for commands, codes, paths and kickers.
- Cards: none. Hairline rows (`.rows`/`.row`), hairline columns (`.pillars`) and a plain table group content. The app window has a hairline and no shadow; its right edge runs off the viewport.
- Video: no `autoplay`. `HeroVideo.astro` plays on intersection and never under `prefers-reduced-motion`; `preload="none"`, the poster carries the message.
- CTA: Apple's official black Mac App Store badge (`public/mac-app-store-badge.svg`) next to a link to the install section, with the OS floor, license and Linux note under it. Install lists the release tarballs with sha256 and `go install`. `brew install ibrahemid/tap/tess` and the `install.sh` curl line return once the tap repo is public and `install.sh` is on `main`.
- Theme: the dark palette applies when the visitor chose it or, with no choice stored, when the system is dark. No `apple-itunes-app` meta: the smart banner is iOS only.
- Pages: `/`, `/cli` (help text from `src/data/cli.json`, regenerated by `scripts/cli-docs.mjs`), `/security`, `/vault-format` (renders `spec/vault-format.md`, its own h1 dropped by a rehype plugin in `astro.config.mjs`), `/vs/ente`, `/vs/authy`, `/changelog`, `/press`, `/learn`, `/support`, `/privacy`, `/404`, and `/og` (rendered to `public/og.png` by `scripts/og.mjs`, excluded from the sitemap). Every page has a real capture in its first screen.
- Gates: `node ~/.claude/copy-gate/scan.mjs src/**/*.astro`, `node ~/.claude/design-gate/scan.mjs dist`, `node ~/.claude/design-gate/render.mjs dist`, `node scripts/check-dist.mjs dist` (also in CI, `.github/workflows/site.yml`).

---

## 12. Redesign, 2026-08-24

Supersedes §11 where they differ. Audit of what the revamp broke: `design/AUDIT.md`; reference lock and decision ledger: `design/LOCK.md`; owed loops: `design/ASSETS.md`.

- One left edge. The page grid is `[full] gap [wide] gap [full]` with `--wide` capped at 1440 px; header (`.shell`), every section, every media block and the footer start at `wide-start`. Text caps itself (`.u-content` 1100 px, `.measure` 60ch); media never does. `.u-bleed-right` runs from `wide-start` to the viewport edge. There is no separate centered content column.
- Hero: copy 5/12, recording 7/12, both inside `wide`; the panel is `aspect-ratio: 16 / 9`, fills its cell and ends where the header ends. Below 1120 px it stacks. Nothing crops, nothing bleeds.
- Recording: `scripts/hero.tape` at 1280x720, 30 px Menlo, 13 rows; five accounts, `tess watch` as frame 0 with full bars (the record script waits for a 30 s window top and allows for vhs startup), then `tess add` from a QR, `tess code github -c`, `--json`, `tess watch` with six. 13.9 s, 190 KB mp4.
- Bands on `/`: ink hero / paper install (two columns, hairline rows) / sunken app (window cut by the viewport edge, overlapping the next band by 3.5rem) / paper one-vault (hairline columns across `wide`) / paper security (hairline rows at 1100, hairline above spanning `wide`) / sunken compare (three columns, stacked under 720 px) / ink close running into the footer with a hairline.
- Type: kickers are body-face small caps, not mono. Mono only for commands, codes, paths and file names. `.row .src` is body text.
- Page heads (`PageHead.astro`): 5/12 copy, 7/12 media right-aligned to `wide-end`; no bleed variant.
- OG: the panel sits whole inside the 1200x630 frame.
- Gates: `node scripts/render-widths.mjs dist --out design/renders/<name>` renders every page at 1440/1920/2560/390 (add `--dark`) and fails on horizontal overflow; plus the four in §11.
