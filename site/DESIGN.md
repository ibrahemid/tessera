...
- Direction: the product is the hero. A dark band carries a real terminal recording of `tess` (vhs against a throwaway vault, `public/media/hero.{mp4,webm}`, poster = frame 0); the app section carries the real window capture (`src/assets/app-vault-{light,dark}.png`, cropped from `docs/appstore-assets/01-vault-*.png`). No CSS mocks: `TileMosaic.astro` and `Terminal.astro` are gone, and with them the tessellation load animation and scroll reveals.
...
- Rhythm on `/`: dark / cream / sunken wide / cream rows / hairline / cream two-column / sunken rows / cream close / dark footer. Heading size varies per section (`.h-display`, `.h-section`, `.h-sub`, `.h-small`). Document pages (`/cli`, `/security`, `/vault-format`, `/privacy`, `/changelog`, `/learn`) keep one h2 size on purpose: their sections are peers.
- Type: two weights only, 400 body (Hanken Grotesk) and 700 display (Schibsted Grotesk); IBM Plex Mono 400 for commands, codes, paths and kickers.
- Cards: none. Hairline rows (`.rows`/`.row`) and hairline columns (`.pillars`) group content. The app window keeps a shadow because it is a window.
- CTA: Apple's official black Mac App Store badge (`public/mac-app-store-badge.svg`) next to a copyable `brew install ibrahemid/tap/tess`, with the OS floor, license and Linux note under it.
- Pages: `/`, `/cli` (help text from `src/data/cli.json`, regenerated by `scripts/cli-docs.mjs`), `/security`, `/vault-format` (renders `spec/vault-format.md`), `/vs/ente`, `/vs/authy`, `/changelog`, `/press`, `/learn`, `/support`, `/privacy`, `/404`. Every page has a real capture in its first screen.
- Gates: `node ~/.claude/copy-gate/scan.mjs src/**/*.astro`, `node ~/.claude/design-gate/scan.mjs dist`, `node ~/.claude/design-gate/render.mjs dist`.
